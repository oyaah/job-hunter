...
2. For each, check acceptance via the bundled LinkedIn server — `mcp__linkedin__get_person_profile` (now 1st-degree / messageable = accepted), or their presence in `mcp__linkedin__get_inbox`. Trust your judgment on the signal; don't over-call.
3. For anyone newly accepted: `linkedin_accepted(contact_id)` → it returns the prepared DM and moves them to DM_REVIEW. If the DM is thin or stale, dispatch `message-writer` to draft a fresh one from their (now richer) profile.
4. Surface the DM to the user for a quick review (same gate as email — capture any edit as a `learning`).
5. On approval: send it via `mcp__linkedin__send_message`, then `linkedin_dm_sent(contact_id)`.
